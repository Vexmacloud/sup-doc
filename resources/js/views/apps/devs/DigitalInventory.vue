<template>
  <div class="onboarding-container">
    <header class="header-section">
      <h1 class="main-heading">Digital Inventory</h1>
        ...
        <div class="content-section">
          <transition name="fade-slide" mode="out-in">
            <div class="step-info" :key="currentStep">
              ...
              <p class="step-description" v-if="steps[currentStep].description">{{ steps[currentStep].description }}</p>
              
              <div class="step-details">
                <transition-group name="list" tag="ul" class="feature-list">
                  <li v-for="(item, index) in steps[currentStep].details || steps[currentStep].bulletPoints" :key="item">
...
export default defineComponent({
  ...
  setup() {
    const steps = ref([
      {
        id: 1,
        title: "Manage & Access Order Files",
        image: "/images/Digital-inventory-client-dashboard.webp",
        description: `The Digital Inventory section serves as a dedicated space where customers can manage and track all files related to their orders. This includes design files, specifications, blueprints, and other essential documents uploaded during the order process.`,
        bulletPoints: [
          "Customers can navigate to the Digital Inventory section to view and manage files linked to their orders.",
          "The system securely stores order-related documents, such as design files, technical drawings, and specifications.",
          "Each file is attached to its respective order, making it easy to track and reference when needed.",
          "Customers can download, review, or re-upload updated versions of their files to ensure smooth processing."
        ],
        tip: "Always double-check your uploaded files for accuracy before submission to avoid delays in production."
      },
    ]);
